with base as (
    ...
    select *
    from {{ ref('stg_linkedin_pages__share_content_tmp') }}
    ...
    select
        {{
            fivetran_utils.fill_staging_columns(
                source_columns=adapter.get_columns_in_relation(ref('stg_linkedin_pages__share_content_tmp')),
                staging_columns=get_share_content_columns()
            )
        }}
